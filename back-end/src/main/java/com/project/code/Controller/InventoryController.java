...
public class InventoryController {
    ...
    public Map<String, String> updateInventory(@RequestBody CombinedRequest request) {
        ...
        System.out.println("Уровень запасов: " + inventory.getStockLevel());
        if (!serviceClass.ValidateProductId(product.getId())) {
            map.put("message", "Id " + product.getId() + " отсутствует в базе данных");
            return map;
        }
        productRepository.save(product);
        map.put("message", "Успешно обновлен продукт с id: " + product.getId());
        if (inventory != null) {
            try {
                ...
                if (result != null) {
                    ...
                    map.put("message", "Нет данных для этого продукта или магазина с id");
                    return map;
                }
            } catch (DataIntegrityViolationException e) {
                map.put("message", "Ошибка: " + e);
    ...
    public Map<String, String> saveInventory(@RequestBody Inventory inventory) {
        ...
        try {
            if (serviceClass.validateInventory(inventory)) {
                ...
                map.put("message", "Данные уже присутствуют в инвентаре");
                return map;
            }
        } catch (DataIntegrityViolationException e) {
            map.put("message", "Ошибка: " + e);
        ...
        map.put("message", "Продукт успешно добавлен в инвентарь");
        return map;
    }
    @GetMapping("/{storeid}")
